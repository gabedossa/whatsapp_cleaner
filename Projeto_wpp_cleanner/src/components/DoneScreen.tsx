...
export default function DoneScreen({ onNavigate, totalCleanedMB }: DoneScreenProps) {
  ...
  return (
    <div className="screen-tight flex flex-1 flex-col items-center justify-center text-center animate-fadeUp">
      ...
      <div className="my-5 font-display text-5xl font-extrabold leading-none text-mint drop-shadow-[0_0_26px_rgba(0,229,160,0.34)]">
        <Counter target={totalCleanedMB} suffix=" MB" duration={1500} />
      </div>

      <section className="glass-card mb-6 w-full p-5 text-left">
      ...
      <button type="button" onClick={() => onNavigate("home")} className="primary-action focus-ring">
        Voltar ao início
      </button>
    </div>
  );
